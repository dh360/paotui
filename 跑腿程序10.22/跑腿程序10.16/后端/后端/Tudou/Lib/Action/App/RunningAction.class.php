...
class RunningAction extends CommonAction{
	...
	public function OrderOrderCommit(){
		...
		$data['coupon_price'] = $data['coupon_price']*100;//红包优惠金额
		
		
		if($data['coupon_price'] <= $data['MoneyFreight']){
		...
		$data['orderType'] = $data['orderType'] ? $data['orderType'] : 1;//订单类型
		
		$data['freight'] = $data['MoneyFreight'];
		
		
		$data['need_pay'] = $data['MoneyPayment'];
	...
	public function OrderOrderPayment(){
		...
		$openid = $Connect['openid'] ? $Connect['openid'] : $Connect['open_id'];
	...
	public function OrderOrderCancel(){
		...
		if(!$res= M('Running')->where(array('running_id'=>$running_id))->save(array('OrderStatus'=>'512'))){
			...
			//D('Users')->addMoney($detail['user_id'],$detail['MoneyPayment'],'跑腿订单用户取消退款退款余额',2,$v['school_id']);//退款给余额
	...
	public function OrderOrderRefund(){
		...
		if($res = M('Running')->where(array('running_id'=>$running_id))->save(array('OrderStatus'=>'512'))){
			...
			$info = '跑腿订单【'.$running_id.'】用户取消退款退款【'.round($detail['MoneyPayment']/100,2).'】元';
			$mix = $this->config['running']['running_weixin_original_refund_mix'] ? $this->config['running']['running_weixin_original_refund_mix'] : 10;
			$mix2 = $mix*100;
		   
			if($this->config['running']['running_weixin_original_refund'] == 1 && $detail['MoneyPayment'] < $mix2  && $detail['MoneyTip'] == 0){
				$runningOrderRefundUser = D('Running')->runningOrderRefundUser($running_id,$detail['user_id'],$detail['MoneyPayment'],'running',$info);
				if($runningOrderRefundUser == false){
					M('Running')->where(array('running_id'=>$running_id))->save(array('OrderStatus'=>8192,'OrderRefundInfo'=>D('Running')->getError()));
					$this->ajaxReturn(array('ErrorCode'=>'E03018','ErrorMessage'=>'退款有一点问题'.D('Running')->getError(),'IsSuccess'=>false));
				}
				$info2 = '操作成功资金原路返回';
			}else{
				D('Users')->addMoney($detail['user_id'],$detail['MoneyPayment'],$info,2,$detail['school_id']);//退款给余额
				$info2 = '操作成功资金退回余额中';
			}
			...
			$this->ajaxReturn(array('ErrorCode'=>'1','SuccessMessage'=>$info2,'IsSuccess'=>true,'Data'=>json_encode($res)));
		}
		$this->ajaxReturn(array('ErrorCode'=>'E03018','ErrorMessage'=>'操作失败','IsSuccess'=>false));
	}
	...
	public function OrderOrderAccept(){
		...
		if($detail['Type'] == 2){
			...
			if($detail['OrderStatus'] == 2 && $cha > ($detail['ExpiredMinutes']*60)){
				...
				//去掉订单改变状态
				$res= M('Running')->where(array('running_id'=>$running_id))->save(array('OrderStatus'=>'2048'));
				
				
				$info = '跑腿订单ID【'.$running_id.'】超时无人接单退款';
				$mix = $this->config['running']['running_weixin_original_refund_mix'] ? $this->config['running']['running_weixin_original_refund_mix'] : 10;
				$mix2 = $mix*100;
				if($this->config['running']['running_weixin_original_refund'] == 1 && $detail['MoneyPayment'] < $mix2  && $detail['MoneyTip'] == 0){
					$runningOrderRefundUser = D('Running')->runningOrderRefundUser($running_id,$detail['user_id'],$detail['MoneyPayment'],'running',$info);
					if($runningOrderRefundUser == false){
						M('Running')->where(array('running_id'=>$running_id))->save(array('OrderStatus'=>8192,'OrderRefundInfo'=>D('Running')->getError()));
						$this->ajaxReturn(array('ErrorCode'=>'E03018','ErrorMessage'=>'订单已经超时退款有一点问题'.D('Running')->getError(),'IsSuccess'=>false));
					}
					$info2 = '操作成功资金原路返回';
				}else{
					D('Users')->addMoney($detail['user_id'],$detail['MoneyPayment'],$info,2,$detail['school_id']);//退款给余额
				...
				$this->ajaxReturn(array('ErrorCode'=>'E03018','ErrorMessage'=>'该订单已经超时无法抢单'.$info2,'IsSuccess'=>false));
			}
		}
	...
	public function HomeMine(){
		...
		$Users['VerifyStatus'] = $RunningDelivery['audit'] ? $RunningDelivery['audit'] : 0;
	...
	public function UserAccountWithdraw(){
        ...
        if(!$detail = M('users')->find($this->uid)){
			$this->ajaxReturn(array('ErrorCode'=>'180','ErrorMessage'=>'会员信息不存在','IsSuccess'=>false));
		}elseif($detail['is_lock'] == 1){
			$this->ajaxReturn(array('ErrorCode'=>'180','ErrorMessage'=>'您的账户已被锁，暂时无法提现','IsSuccess'=>false));
		}
		...
		/*
		$bg_time = strtotime(TODAY);
		$count = M('UsersCash')->where(array('user_id'=>$user_id,'type'=>user,'addtime'=>array(array('ELT',NOW_TIME),array('EGT',$bg_time))))->count();
		if($count){
			$this->ajaxReturn(array('ErrorCode'=>'180','ErrorMessage'=>'每天只能提现一次【'.$count.'】','IsSuccess'=>false));
		}
		*/
   ...
   public function ThreadGetSensitive(){
	...
	public function ThreadAd(){
		...
		$site_id = $this->config['wxapp']['thread_site_id'] ? $this->config['wxapp']['pthread_site_id'] : '85';
		if($school_id){
			$list = M('ad')->where(array('site_id'=>$site_id,'closed'=>'0','school_id'=>$school_id))->select();
		}else{
			$list = M('ad')->where(array('site_id'=>$site_id,'closed'=>'0'))->select();
	...
	public function ThreadType(){
		$arr = M('ThreadCate')->limit(0,10)->select();
	    foreach($arr as $k => $v){
		   $arr[$k]['id'] = $v['cate_id'];
	...
	public function ThreadLike(){
		...
		$res = M('ThreadPostZan')->where(array('post_id' => $post_id,'user_id' => $user_id,'school_id'=>$school_id,'create_ip' => get_client_ip()))->find();
        if($res){
             echo '请不要重复点赞';
        }else{
            if(M('ThreadPostZan')->add(array('post_id'=>$post_id,'user_id'=>$user_id,'school_id' =>$school_id,'create_time' => time(),'create_ip' => get_client_ip()))) {
   ...
   public function ThreadComments(){
	...
	public function ThreadMyPost(){
		...
		$list = M('ThreadPost')->where($map)->limit($Page->firstRow.','.$Page->listRows)->order(array('top_date'=>'desc','is_fine'=>'desc','create_time'=>'desc'))->select();
        foreach($list as $k => $val){
			$Users = M('users')->find($val['user_id']);
			$list[$k]['id'] = $val['post_id'];
		    $list[$k]['user'] = $Users;
	...
	public function GroupPay(){
		...
		$connect = M('connect')->where(array('openid'=>$openid))->find();
		if(!$connect){
	...
	public function CouponList2(){
		...
		foreach($list as $k => $val){
			...
			$list[$k]['reduce'] =  round($val['reduce_price']/100,2);
	...
	public function LqCoupon(){
		...
		$count =(int) M('coupon_download')->where(array('coupon_id'=>$coupon_id,'user_id'=>$user_id ))->count();
		if($count){
	...
	public function Pay5(){
		...
		$users = M('users')->where(array('user_id'=>$user_id))->find();
		$openid = $openid ? $openid : $users['openid'];

...
class WeixintmplModel extends CommonModel{
	...
	public function runningNoticeDelivery($running_id = 0){
		...
		//file_put_contents(BASE_PATH.'/Tudou/Lib/Model/$detail.txt', var_export($detail,true));
		//file_put_contents(BASE_PATH.'/Tudou/Lib/Model/$list.txt', var_export($list,true));
		...
		foreach($list as $k=>$v){
			...
			/*
			p('当前时间--'.$time.'开始时间--'.$notifyFrom.'结束时间--'.$notifyEnd);
			p($detail['cate_id']);	
			p($notifyFlag);
			p($notify);
			p($times);die;
	...
	public function sendWxappNotice($detail,$openid,$form,$formid1,$user_id = 0,$v = array()){
		...
		if($tpmlMsgType == 0){
			...
			//配送员公众号模板消息通知
			D('Weixinmsg')->wxTmplRunningDeliveryMsg($detail['running_id'],$detail['OrderStatus'],$detail['user_id'],1,$openid='',$form_id='');
		}
		return false;
	}

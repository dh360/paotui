	...
	public function updateOrder($order_id,$order_ids,$code){
		 if(empty($order_id) && !empty($order_ids)){
			 //合并付款
              ...
              D('Order')->save(array('status' =>1,'code'=>$code), array('where' => array('order_id' => array('IN', $order_ids))));
			...
			D('Order')->save(array('order_id' =>$order_id,'status'=>1,'code'=>$code));
			$Order = D('Order')->find($order_id);
			$Shop = D('Shop')->find($Order['shop_id']);
			$ConnectUser = D('Connect')->where(array('uid'=>$Order['user_id'],'type'=>'weixin'))->find();
			$ConnectShop = D('Connect')->where(array('uid'=>$Shop['user_id'],'type'=>'weixin'))->find();
				...
				if($this->save(array('status' => 8, 'order_id' => $order_id))){
					...
					list($settlement_price,$intro) = $this->get_order_settlement_price_intro($detail);//获取结算价封装
						...
						if($detail['is_daofu'] == 0){
							...
							//威望返还给用户
							if($Shop['is_restore_prestige'] == 1 && $config['prestige']['is_goods']){

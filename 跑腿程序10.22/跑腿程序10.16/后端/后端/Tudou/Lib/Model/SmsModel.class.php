...
class SmsModel extends CommonModel{
    ...
    public function runningPayUser($running_id){
		...
		//file_put_contents(BASE_PATH.'/Tudou/Lib/Model/$Running.txt', var_export($Running,true));
		...
		if($Running['Type'] == 1){
			//file_put_contents(BASE_PATH.'/Tudou/Lib/Model/eleTZshop.txt', var_export($Running,true));
    ...
    public function sms_delivery_user($running_id){
		...
		foreach($list as $k=>$v){
			$users = M('users')->find($v['user_id']);//新用户账户
			$mobile = $v['phoneNumber']?$v['phoneNumber']:$users['mobile'];
			if($mobile){
				$this->send('sms_delivery_user',$shop_id = '0',$mobile,array(
					'userName' => niuMsubstr($users['niakname'],0,8,false),

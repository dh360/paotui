...
class WeixinmsgModel extends CommonModel{
	...
	//公众号抢单配送批量发送微信模板消息wxTmplRunningDeliveryMsg
	public function wxTmplRunningDeliveryMsg($running_id,$OrderStatus,$user_id= '',$type=1,$openid='',$form_id=''){
		...
		$arr = M('running_delivery')->where($map)->select();
		...
		foreach($arr as $val){ 
			if($val['RealName']){
				$array = array(
					...
					'first' => '尊敬的【'.$val['RealName'].'】，配送中心有新的订单等您去抢单哦',
		  ...
		  $return = $this->net($val['user_id'],'OPENTM400045127',$data);
       } 
       return true;
    }
	...
	public function net($uid,$serial=null,$data=null){
		...
		if($openid){
			...
			$html = 'uid_【'.$uid.'】open_id_【'.$openid.'】template_id_【'.$data['template_id'].'】<br>';
